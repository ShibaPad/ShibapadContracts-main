...
contract SPS_STAKING is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    function initializeNFT(address _address) public {
    NFTcontract = SPS(_address);
    }

    function getRarity(uint256 _tokenId) public view returns (uint256) {
    return NFTcontract.getrarity(_tokenId);
    }

    function getRarityRewardrate(uint256 _tokenId) private {
    if (getRarity(_tokenId)==1){
    rewardRate[_tokenId] = getRarity(_tokenId) * R1;
    }
    else if (getRarity(_tokenId)==2){
    rewardRate[_tokenId] = getRarity(_tokenId) * R2;
    }
    else if (getRarity(_tokenId)==3){
    rewardRate[_tokenId] = getRarity(_tokenId) * R3;
    }
    else if (getRarity(_tokenId)==4){
    rewardRate[_tokenId] = getRarity(_tokenId) * R4;
    }
    else if (getRarity(_tokenId)==5){
    rewardRate[_tokenId] = getRarity(_tokenId) * R5;
    }

    }

    function setRarityRewardrate(uint256 _R1, uint256 _R2, uint256 _R3, uint256 _R4, uint256 _R5) external onlyOwner{
    R1 = _R1;
    R2 = _R2;
    R3 = _R3;
    R4 = _R4;
...
/* STAKING MECHANICS */
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        rewards[i] = 
          ...
          (Math.min(block.number, expiration) - 
            _depositBlocks[account][tokenId]) * (getRarity(tokenId));
      }
    ...
    function deposit(uint256[] calldata tokenIds) external whenNotPaused {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (getRarity(tokenIds[i]) > Tiers[msg.sender]){
            Tiers[msg.sender] = getRarity(tokenIds[i]);
            }
    }
    }
